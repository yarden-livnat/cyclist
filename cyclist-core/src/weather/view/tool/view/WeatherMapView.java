	...
	private void drawWeather() {
		...
		for (Intersect intersect : world.getIntersects()) {
			...
			if (station.getBBox().intersects(_worldBBox)) {
				...
				Color color = getColor(data.getDryBulb());
				gc.setFill(color);
				gc.setStroke(color);
				for (Node node : intersect.getNodes()) {
					double x = Math.round(node.getX()*10_000+0.2)/10_000.0;
					double y = Math.round(node.getY()*10_000+0.2)/10_000.0;
//					if (x != node.getX()) 
//						System.out.println("x: "+node.getX()+"  "+x);
//					if (y != node.getY()) 
//						System.out.println("y: "+node.getY()+"  "+x);
					gc.fillRect(/*node.getX()*/x-Node.NODE_SIZE/2, /*node.getY()*/ y-Node.NODE_SIZE, Node.NODE_SIZE, Node.NODE_SIZE);
					gc.strokeRect(x-Node.NODE_SIZE/2, y-Node.NODE_SIZE, Node.NODE_SIZE, Node.NODE_SIZE);
				}
			}
		}
	}
